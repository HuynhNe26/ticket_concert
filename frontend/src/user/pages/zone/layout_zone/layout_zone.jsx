...
export default function LayoutZone({ layout, zones, eventId }) {
  ...
  if (!layout) {
    return <div className="layout-loading"><p>Chưa cập nhật!</p></div>;
  }
  ...
  const handleCanvasClick = (e) => {
    ...
    for (const zone of mergedZones) {
      ...
      if (zone.shape === 'rect') {
        inside = x >= zone.x && x <= zone.x + zone.width && y >= zone.y && y <= zone.y + zone.height;
  ...
  const handleAddToCart = async (eventId, zone_id, quantity) => {
    ...
    try {
      const response = await fetch(`${API_BASE}/api/cart/${eventId}`, {
        ...
        body: JSON.stringify({ zone_id, eventId, quantity })
      });

      const data = await response.json();

      if (!response.ok) {
        showError(data.message || 'Thêm vào giỏ thất bại');
        return;
      }

      showSuccess('Đã thêm vào giỏ hàng!');
      setTimeout(() => navigate('/my-cart'), 1000);
    ...
    <div className="layout-zone-wrapper">
      ...
      {showOverlay && selectedZone && (
        <div className="zone-overlay">
          <div className="zone-modal">
            ...
            <p className="zone-desc">{selectedZone.description || 'Chưa cập nhật!'}</p>
            ...
            <div className="zone-quantity">
              ...
              <div>
                ...
                <button
                  disabled={quantity >= 5 || quantity >= selectedZone.available}
                  onClick={() => setQuantity(q => Math.min(5, selectedZone.zone_quantity - selectedZone.sold_quantity, q + 1))}
                >
                  +
                </button>
              </div>
            ...
            <div className="zone-actions">
              <button className="btn-cancel" onClick={() => setShowOverlay(false)}>Huỷ</button>
              <button className="btn-confirm" onClick={() => handleAddToCart(eventId, selectedZone.zone_id, quantity)}>
      ...
      {showLoginModal && (
        <div style={{
          ...
          <div style={{ position: 'relative', maxWidth: '350px', width: '90%', maxHeight: '90vh', overflow: 'auto' }}
